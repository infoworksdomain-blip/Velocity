...
const PROVIDER_ID = "seedance-2.5";

export function createSeedanceStubProvider(tiers: string[]): VideoProvider {
  const store = new DeterministicJobStore(PROVIDER_ID, {
    processingMs: 250,
    costUsd: (input) => (input as VideoJobInput).durationSec * 0.08,
  });
