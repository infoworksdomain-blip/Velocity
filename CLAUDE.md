...
- Migration 0001's tenant-table RLS/grant loops each ran once, over tables that existed at that time only — its own comment says so. Every later migration that adds a workspace-scoped table (0006_notifications is the first example) must hand-write its own `ENABLE`/`FORCE ROW LEVEL SECURITY` + `workspace_isolation` policy + `GRANT`, or that table silently has no tenant isolation.

Next: STEP 8 (Content Engine).
